...
//Functie om alle 'a' elementen te verwijderen. Hiermee verwijder je alle films op het scherm
const parentOfLink = document.querySelector('.list-of-movies');
const empty = () => {
	parentOfLink.querySelectorAll('a').forEach((empty) => empty.remove());
};

//Functie om 'a' elementen met 'href' toe te voegen  en een 'img' + 'src' voor de posters
const addMovies = (mov) => {
	const movieArray = mov; // OPMERKING: is trikt genomen niet nodig denk ik. Kan in volgende 'mov' gebruiken ipv 'movieArray'. Maakt het misschien wel duidelijker
...
//Alle films
const buttonAllMovies = document.querySelector('#all-movies'); // OPMERKING: deze bestaat volgens mij niet en is in dat geval niet nodig

// OPMERKING: volgens mij zorgt deze functie ervoor dat alle films op het scherm komen. Dan is de naam van de functie wat verwarrend
...
//OPMERKING: button slaat vaak op een 'submitbutton'. In dit geval is radio-button. Kan wat verwarrend zijn. Denk dat 'radioButtonFor2014' prima naam is
...
const searchMovies2014 = () => {
	empty(), clearSearchArea(), addMovies(movies2014);
};
...
buttonForTitleMovies.forEach((button, index) => {
	...
	const searchMovies = () => {
		empty(), clearSearchArea(), addMovies(movieRight);
	};
...
selected.forEach((select) => {
	// console.log('select.check is', select.checked);
	// console.log(select, 'checked');
	select.addEventListener('change', () => {
		const selected = document.querySelectorAll('.selected');
		selected.forEach((select) => {
			if (select.checked) {
				console.log('select.checked', select.checked);
				console.log('select.parentElement is', select.parentElement);
				select.parentElement.classList.add('checked');
				select.parentElement.classList.remove('not-checked');
			}
			if (!select.checked) {
				console.log('select.checked', select.checked);
				console.log('select.parentElement is', select.parentElement);
				select.parentElement.classList.remove('checked');
				select.parentElement.classList.add('not-checked');
			}
		});
	});
});
